...
VoxLink 是面向 Windows 10/11 x64 的双向实时语音翻译桌面应用，为 VRChat 等多人游戏设计：识别你的麦克风与系统输出语音，把译文发送到 VRChat Chatbox，或经虚拟声卡把译后语音送入 VRChat 麦克风。
...
- **双向翻译**：麦克风（我的声音）与系统回环（对方声音）各走独立识别通道
...
- **免密翻译故障转移** + 可选 LLM 润色与第二目标语言（Chatbox 同屏显示主、次译文）
- 桌面悬浮字幕与可选 SteamVR 头显字幕；面向简体中文统一字形
- 可选说话人标签（关闭 / 本地匿名聚类 / 云端 speaker ID）
- 跟随 VRChat `MuteSelf` 暂停麦克风；启动后台检查更新
- API Key 与自定义请求头用当前 Windows 用户 DPAPI 加密保存
- 内置「日志」页排查翻译与引擎问题，日志同时写入磁盘

## 安装

从 [Releases](https://github.com/znc15/VoxLink/releases) 下载，二选一：

- **安装包** `Setup-VoxLink-x.x.x.exe`：按当前用户安装到 `%LOCALAPPDATA%\Programs\VoxLink`，无需管理员（未签名，首次可能触发 SmartScreen）
- **便携包** `VoxLink-win-x64.zip`：解压后运行包根目录的 `VoxLink.exe`

两种包都自带 .NET 运行时与 Windows App SDK。首次启动按引导选择模式、语言与设备即可开始。
...
普通设置在 `%APPDATA%\VoxLink\settings.json`；API Key 与请求头以当前用户 DPAPI 加密保存在 `%APPDATA%\VoxLink\secrets.dat`；日志在 `%APPDATA%\VoxLink\logs\`。默认快捷键：`Ctrl+Alt+Space` 开始/停止，`Ctrl+Alt+Enter` 翻译当前输入。
...
要求：Windows 10 2004+ x64、.NET 10 SDK、NuGet 可达。
...
涉及真实 Whisper / WASAPI / VRChat / SteamVR / 云服务的用例仅在 `VOXLINK_RUN_LIVE_TESTS=1` 时执行（默认 mock，不需要 Key）。发布：
...
**约定**：提交信息用中文约定式提交 `类型(范围): 描述`（feat/fix/docs/refactor/test/chore）；功能或修复有实质变化时同步 `src/VoxLink.UI/VoxLink.UI.csproj` 的 `<Version>`，发布时打 `vX.Y.Z` 标签，CI 自动构建并发布 Release。启动后台查询 GitHub Releases 检查更新，高级设置页可手动检查。
...
WinUI 3 / Windows App SDK 2.3.1 工作台 + .NET 10 独立 Engine（WASAPI 采集、Whisper.net、sherpa-onnx 说话人、DashScope/Soniox/OpenAI/MiMo、Edge/Google/Windows TTS、VRChat OSC、OpenVR 字幕、DPAPI 存储）。详见 [docs/architecture.md](docs/architecture.md)。MIT License；参考项目（realtime-subtitle、VRCTTP）为 AGPL-3.0，本实现为独立清洁实现。
