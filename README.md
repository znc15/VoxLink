...
- **麦克风语音增强**：对用户输入语音做降噪与自动增益（AGC），提升语音识别准确率；可在「音频设备」页切换 WebRTC / RNNoise / 关闭；实现与开源选型说明见 [docs/speech-preprocessing.md](docs/speech-preprocessing.md)
...
- **桌面与 VR 字幕**：桌面悬浮窗支持拖动、拉伸、置顶或固定，可永久显示或按设定秒数自动隐藏；可选用 SteamVR 头显字幕
...
| 音频设备 | 采集源、设备路由、断句、麦克风语音增强（WebRTC / RNNoise 可切换） |
...
| 高级设置 | 会话行为（仅转写说明）、快捷键、窗口外观、本地模型存储位置（可用系统文件夹选择器） |
| 关于 | 版本、更新、运行状态 |
| 日志 | 运行日志 |
